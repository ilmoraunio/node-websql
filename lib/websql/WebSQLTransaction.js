...
function runAllSql(self) {
  ...
  if (self._error || (!self._sqlQueue.length && self._commitable)) {
    self._complete = true;
    return self._websqlDatabase._onTransactionComplete(self._error);
  }
  if (!self._commitable && !self._sqlQueue.length) {
    return;
  }
  self._running = true;
  var batch = [];
...
function WebSQLTransaction(websqlDatabase) {
  ...
  this._commitable = true;
  this._readOnly = websqlDatabase._currentTask.readOnly;
...
WebSQLTransaction.prototype.commit = function () {
  if (this._readOnly) {
    throw new Error("Explicit commit not supported for read-only transactions.");
  }
...
WebSQLTransaction.prototype.rollback = function (err) {
  if (this._readOnly) {
    throw new Error("Explicit rollback not supported for read-only transactions.");
  }
...
WebSQLTransaction.prototype.setAutoCommit = function (value) {
  if (this._readOnly) {
    throw new Error("Disabling auto-commit not supported for read-only transactions.");
  }

  this._commitable = value;
};
